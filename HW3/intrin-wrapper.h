...
template <> class alignas(sizeof(double)*4) Vec<double,4> {
  ...
  public:
	...
	template <class Vec> friend Vec RoundReal2Floor(const Vec& x);
...
template <> inline Vec<double,4> RoundReal2Floor(const Vec<double,4>& x) {
  Vec<double,4> r;
  r.v = _mm256_round_pd(x.v,_MM_FROUND_TO_ZERO |_MM_FROUND_NO_EXC);
  return r;
}
