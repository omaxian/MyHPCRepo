...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  ...
  long iBlocks = m/BLOCK_SIZE;
  long jBlocks = n/BLOCK_SIZE;
  long kBlocks = k/BLOCK_SIZE;
  #pragma omp parallel for
  for (long jB=0; jB < jBlocks; jB++){
   for (long kB=0; kB < kBlocks; kB++){
    for (long iB=0; iB < iBlocks; iB++){
     // Do the matrix-matrix multiply by block
     long iStart=iB*BLOCK_SIZE;
     long jStart=jB*BLOCK_SIZE;
     long kStart=kB*BLOCK_SIZE;
     for (long j = jStart; j < jStart+BLOCK_SIZE; j++) {
      for (long p = kStart; p < kStart+BLOCK_SIZE; p++) {
       for (long i = iStart; i < iStart+BLOCK_SIZE; i++) {
	double A_ip = a[i+p*m];
	double B_pj = b[p+j*k];
	double C_ij = c[i+j*m];
	C_ij = C_ij + A_ip * B_pj;
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    printf("%10d %10f %10f %10f", p, time, flops*1e-9, bandwidth*1e-9);
