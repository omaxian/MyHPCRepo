...
#pragma omp parallel shared(a, b, nthreads, locka, lockb) private(tid)
  ...
  #pragma omp sections nowait
    ...
    #pragma omp section
      ...
      for (i=0; i<N; i++)
        a[i] = i * DELTA;
      // There was a deadlock here that happened because this thread locked a, 
      // so thread 2 could never get to a until this thread got to b. But thread
      // 2 put a lock on b, so this thread could never get to b. 
      // To fix this, unset the locks after the arrays are written. 
